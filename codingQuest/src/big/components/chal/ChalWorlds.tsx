...
const WorldsMap: React.FC<WorldsMapProps> = ({ completedWorlds }) => {
  ...
  const worlds = [
    ...
    { id: 2, title: 'Estruturas de Controle', description: 'Condicionais e loops' },
    { id: 3, title: 'Funções e Objetos', description: 'Abstração de código' },
    { id: 4, title: 'Estruturas de Dados', description: 'Arrays, listas e dicionários' },
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="text-center mb-8 h-23">
        ...
        <p className="text-white text-base">Selecione um mundo e resolva seus desafios!</p>
      </div>
      <div className="px-10 h-70">
        <Slider {...sliderSettings}>
          {worlds.map((world) => {
            const isUnlocked = completedWorlds >= world.id - 1;
            ...
            return (
              <div key={world.id} className="px-2 focus:outline-none">
                ...
                >
                  <div className="font-bold mb-2 text-[#24C97B]">{world.id}° Mundo</div>
                  {isUnlocked ? (
                    <>
                      <h3 className="text-lg font-semibold mb-2 text-white">
                        {world.title}
                      </h3>
                      <p className="text-gray-300 mb-4 px-4">{world.description}</p>
                    ...
                    <div className="mt-4 flex flex-col items-center justify-center h-full">
                      ...
                      />
                      <p className="text-sm text-gray-400">Complete o mundo anterior para desbloquear</p>
                    </div>
                  )}
                </div>
              </div>
