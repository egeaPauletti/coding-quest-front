export type ContinentSpec = {
  path: string;
  offset: number; // 0-1 range representing horizontal position
  scale: number;
  rotation: number;
  verticalOffsetPercent: number;
};

//SVGs dos continentes
...
const normalizeSeed = (seed: number) => {
  const sanitized = Number.isFinite(seed) ? Math.floor(seed) : 0;
  let value = sanitized + 0x6d2b79f5;
  value |= 0;
  value ^= value >>> 15;
...
const pickInRange = (rand: () => number, min: number, max: number) =>
  min + rand() * (max - min);

export const generateContinents = (seed: number, count: number): ContinentSpec[] => {
  const safeCount = Math.max(1, Math.min(count, 8));
  const rand = mulberry32(seed);
  const baseSpacing = 1 / safeCount;

  const continents = Array.from({ length: safeCount }, (_, index) => {
    const jitter = (rand() - 0.5) * baseSpacing * 0.6;
    const offset = (index * baseSpacing + jitter + 1) % 1;

    return {
      path: CONTINENT_PATHS[index % CONTINENT_PATHS.length],
      offset,
      scale: pickInRange(rand, 0.7, 1.2),
      rotation: pickInRange(rand, -20, 20),
      verticalOffsetPercent: pickInRange(rand, -30, 30),
    };
  });

  return continents.sort((a, b) => a.offset - b.offset);
};
